...
const loadImmHandler: ExecutionHandler = (state, [rA, imm]) => {
    ...
    // Debug PC=17438
    if (state.pc === 17438) {
        console.log(`[loadImm PC=17438] rA=${rA} imm=0x${BigInt(imm).toString(16)} (${imm})`);
    }
...
const storeImmInd = (bytes: 1 | 2 | 4 | 8): ExecutionHandler =>
    (s, [rA, immX, immY]) => {
        ...
        // Debug step 12 (PC=18943)
        if (s.pc === 18943) {
            console.log(`[DEBUG PC=18943] storeImmInd: rA=${rA}, immX=${immX}, immY=${immY}, bytes=${bytes}`);
            console.log(`[DEBUG PC=18943] r[${rA}]=0x${s.registers[rA].toString(16)}, r7=0x${s.registers[7].toString(16)}`);
        }
        // Debug PC=21689 (writes to 0x32788)
        if (s.pc === 21689) {
            const base = Number(s.registers[rA]);
            const addr = base + Number(immX);
            console.log(`[DEBUG PC=21689] storeImmInd: rA=${rA}, immX=0x${Number(immX).toString(16)}, immY=0x${Number(immY).toString(16)}`);
            console.log(`[DEBUG PC=21689] r[${rA}]=0x${s.registers[rA].toString(16)}, addr=0x${addr.toString(16)}`);
        }
...
// opcode 141: negate and add immediate 32 bits - result sign-extended to 64 bits
const negAddImm32Handler = twoRegImmOp((rB, imm) => {
    const result32 = (BigInt(imm) + (1n << 32n) - (rB & 0xFFFFFFFFn)) & 0xFFFFFFFFn;
    return BigInt.asIntN(64, BigInt.asIntN(32, result32));  // sign-extend 32 to 64 bit
}); // 141
...
// 219 - cmov_nz: φ′D = φA if φB ≠ 0, otherwise φD
// Graypaper A.31: operands are [rA, rB, rD] where rB is condition, rA is source, rD is dest
const cmovNzHandler: ExecutionHandler = (s, [rA, rB, rD]) => {
    const regs = s.registers.slice();
    if (regs[rB] !== 0n) regs[rD] = regs[rA];  // if φB ≠ 0, then φ′D = φA
    return {
        ...s, registers: regs, pc: nextPc(s),
        // gas: s.gas - GAS_PER_INSTRUCTION 
    };
...
// Rotate handlers - must mask 'a' to unsigned BEFORE right shift because BigInt >> is arithmetic (sign-extending)
const MASK_64 = 0xFFFF_FFFF_FFFF_FFFFn;
const MASK_32 = 0xFFFF_FFFFn;
const rotL64Handler = threeRegOp((a, b) => {
    const au = a & MASK_64;  // ensure unsigned
    const shift = b % 64n;
    return ((au << shift) | (au >> (64n - shift))) & MASK_64;
}); // 220
const rotL32Handler = threeRegOp((a, b) => {
    const au = a & MASK_32;  // ensure unsigned 32-bit
    const shift = b % 32n;
    return ((au << shift) | (au >> (32n - shift))) & MASK_32;
}); // 221
const rotR64Handler = threeRegOp((a, b) => {
    const au = a & MASK_64;  // ensure unsigned
    const shift = b % 64n;
    return ((au >> shift) | (au << (64n - shift))) & MASK_64;
}); // 222
const rotR32Handler = threeRegOp((a, b) => {
    const au = a & MASK_32;  // ensure unsigned 32-bit
    const shift = b % 32n;
    return ((au >> shift) | (au << (32n - shift))) & MASK_32;
}); // 223
