...
// aggregator wrapper
export function aggregator(ringKeys: string | string[], ringSize: number, srsPath: string): Uint8Array {
  ...
  const srsPathBuf  = new Uint8Array(Buffer.from(srsPath + "\0", "utf8"));
...
export function ringVrfSign(
  ...
  const secretBuf   = new Uint8Array(Buffer.from(secretHex + "\0", "utf-8"));
  const ringKeysBuf = new Uint8Array(Buffer.from(ringKeys, "utf-8"));
  const srsBuf      = new Uint8Array(Buffer.from(srsPath + "\0", "utf-8"));
...
if (import.meta.main) {
  try {
    ...
    const result = aggregator(ringKeysStr, ringSize, srsPathStr);
