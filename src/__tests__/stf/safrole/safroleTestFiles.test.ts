...
describe("SafroleStfCodec roundtrip", () => {
  ...
  testFiles.forEach((fileName) => {
    it(`encodes/decodes from ${fileName}`, () => {
      // 1) load JSON
      const filePath = path.join(__dirname, "../..../stf/safrole/data/tiny", fileName.concat('.json'));
      ...
      const binPath = path.join(`${JAM_TEST_VECTORS}/safrole`, `${CHAIN_TYPE}`, fileName.concat(".bin"));
