...
testFiles.forEach((fileName) => {
  describe("ReportsCodec", () => {
    ...
    it(`encodes/decodes an entire Reports object from ${fileName}`, () => {
      ...
      const filePath = path.join(path.join(`${JAM_TEST_VECTORS}/stf/reports`, `${CHAIN_TYPE}`,
      ...
      console.log("here we are encoded the reports", toHex(encoded));
      ...
      writeFileSync(
        ...
        stringifyJSONSafe(readableDecoded)
      );
      writeFileSync(
        path.join(outDir, "originalReports.json"),
        stringifyJSONSafe(readableOriginal)
      );
  ...
  //     const binPath = path.join(path.join(`${JAM_TEST_VECTORS}/stf/reports`, `${CHAIN_TYPE}`, fileName.concat(".bin"))
  //     );
...
export function stringifyJSONSafe(value: any, space = 2): string {
  const toHex = (u8: Uint8Array) =>
    '0x' + Array.from(u8).map(b => b.toString(16).padStart(2, '0')).join('');
  return JSON.stringify(
    value,
    (_k, v) => {
      if (typeof v === 'bigint') return v.toString();      // decimal string
      if (v instanceof Uint8Array) return toHex(v);         // keep bytes readable
      return v;
    },
    space
  );
